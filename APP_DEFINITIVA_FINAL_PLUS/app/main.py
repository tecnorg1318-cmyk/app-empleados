...
<!-- MI JORNADA DETALLADA -->
<div id="mi_jornada_detallada" style="margin-top:12px;background:#0f172a;padding:10px;border-radius:10px"></div>
<!-- MIS RETARDOS -->
<div id="mi_retardos_box" style="margin-top:12px;background:#1e293b;padding:10px;border-radius:10px;border:1px solid #f59e0b22"></div>
<!-- MI HISTORIAL CALIFICACIONES ABAJO DE RETARDOS Y JORNADA -->
<div id="mi_calif_historial" style="margin-top:12px;background:#0f172a;padding:10px;border-radius:10px;border:1px solid #6366f122">
<h4 style="font-size:12px;color:#a78bfa">⭐ Mi Historial de Calificaciones (Admin me evaluó)</h4>
<div id="mi_calif_lista" style="margin-top:8px"></div>
<div id="mi_calif_promedio" style="margin-top:8px;padding:8px;background:#1e293b;border-radius:8px;text-align:center"></div>
</div>
</div>
<div class="card"><h3>Mi info completa + Mi jornada + Retardos + Calificaciones</h3><div id="mi_info"></div><button class="btn btn-g" onclick="checkIn()" style="margin-top:10px">✅ Check-In</button><p style="font-size:10px;color:#64748b;margin-top:6px">Si llegas después de tu horario entrada, se cuenta retardo automático. Si no haces check, al final del día es falta (lo ve admin).</p>
<div style="margin-top:12px;background:#0f172a;padding:10px;border-radius:10px;border:1px dashed #6366f1"><h4 style="font-size:11px">⭐ Calificación más reciente</h4><div id="mi_ultima_calif"></div></div>
</div>
</div>
</div>
...
async function loginEmpleado(){
 ...
 <div style="margin-top:8px;font-size:10px;background:#1e293b;padding:8px;border-radius:8px"><b>Semana:</b><br>${Object.entries(miActual.semana_completa||{}).map(([k,v])=>`${k}: ${v||'Descanso'}`).join('<br>')||'No asignada'}</div>
 </div>`;
 // MI JORNADA DETALLADA ABAJO
 document.getElementById('mi_jornada_detallada').innerHTML=`<h4 style="font-size:12px;color:#10b981">📅 Mi Jornada Hoy</h4><p style="font-size:11px;margin-top:6px">📍 Sucursal: <b>${miActual.sucursal_nombre}</b><br>📅 Fecha: ${miActual.fecha}<br>⏰ Horario: ${miEmpleado.horario_entrada} - ${miEmpleado.horario_salida}<br>🏢 Origen asignación: ${miActual.origen}</p>`;
 ...
 <div style="margin-top:12px;background:#0f172a;padding:10px;border-radius:10px">
 <h4 style="font-size:11px;color:#a78bfa">⭐ Mis evaluaciones (4) - Vista rápida</h4>
...
async function cargarMisVacEval(){
 ...
 // Vista rapida en mi_info
 document.getElementById('my_eval_list').innerHTML=evals.slice(0,3).map(e=>`<div style="font-size:11px;padding:6px;border-bottom:1px solid #1e293b">📅 ${e.fecha} - ⭐ ${e.calificacion}/10<br><span style="color:#94a3b8">${e.comentario}</span></div>`).join('') || '<p style="font-size:10px;color:#64748b">Sin evaluaciones aún</p>';
 
 // MIS RETARDOS BOX - abajo de jornada
 try{
   let visitas=await api('/visitas?empleado_id='+miEmpleado.id);
   let retardos=visitas.filter(v=>v.es_retardo);
   document.getElementById('mi_retardos_box').innerHTML=`<h4 style="font-size:12px;color:#f59e0b">⏰ Mis Retardos y Asistencias (últimos 10)</h4>
   <p style="font-size:11px;margin-top:6px">Total Faltas: <b style="color:#ef4444">${miEmpleado.faltas}</b> | Retardos: <b style="color:#f59e0b">${miEmpleado.retardos}</b></p>
   <div style="margin-top:8px;max-height:200px;overflow:auto">${visitas.slice(0,10).map(v=>`<div style="font-size:10px;padding:4px;border-bottom:1px solid #334155">${v.fecha} ${v.hora_entrada} - ${v.sucursal_nombre} ${v.es_retardo?'🔴 Retardo':'🟢 A tiempo'}</div>`).join('')||'Sin registros'}</div>`;
 }catch(e){ document.getElementById('mi_retardos_box').innerHTML='<p style="font-size:10px;color:#64748b">No se pudo cargar retardos</p>'; }

 // HISTORIAL CALIFICACIONES COMPLETO ABAJO DE MI JORNADA Y RETARDOS
 let califLista=document.getElementById('mi_calif_lista');
 let califProm=document.getElementById('mi_calif_promedio');
 let ultima=document.getElementById('mi_ultima_calif');
 if(evals.length>0){
   // Ordenar por fecha descendente
   evals.sort((a,b)=> new Date(b.fecha)-new Date(a.fecha));
   califLista.innerHTML=evals.map((e,i)=>`<div style="padding:8px;border-bottom:1px solid #1e293b;border-left:3px solid ${e.calificacion>=8?'#10b981':e.calificacion>=6?'#f59e0b':'#ef4444'};margin-bottom:6px;background:#1e293b;border-radius:8px">
   <div style="display:flex;justify-content:space-between"><span style="font-size:11px;color:#94a3b8">📅 ${e.fecha}</span><span style="font-weight:800;color:${e.calificacion>=8?'#34d399':e.calificacion>=6?'#fbbf24':'#f87171'}">⭐ ${e.calificacion}/10</span></div>
   <p style="font-size:11px;margin-top:4px;color:#e2e8f0">${e.comentario||'Sin comentario'}</p>
   <small style="font-size:9px;color:#64748b">Evaluador: ${e.evaluador||'admin'} | #${i+1}</small>
   </div>`).join('');
   let promedio=(evals.reduce((s,x)=>s+x.calificacion,0)/evals.length).toFixed(1);
   califProm.innerHTML=`<p style="font-size:12px">Promedio general: <b style="font-size:18px;color:#a78bfa">⭐ ${promedio}/10</b> de ${evals.length} evaluaciones</p><div style="margin-top:6px;background:#0f172a;height:8px;border-radius:10px"><div style="width:${promedio*10}%;background:linear-gradient(90deg,#6366f1,#a78bfa);height:8px;border-radius:10px"></div></div>`;
   ultima.innerHTML=`<div style="padding:8px;background:#1e293b;border-radius:8px"><b style="color:#a78bfa">⭐ ${evals[0].calificacion}/10</b> - ${evals[0].fecha}<br><small style="color:#94a3b8">${evals[0].comentario}</small></div>`;
 } else {
   califLista.innerHTML='<p style="font-size:11px;color:#64748b">Aún no tienes calificaciones. El admin te evaluará pronto.</p>';
   califProm.innerHTML='<p style="font-size:11px;color:#64748b">Promedio: 0</p>';
   ultima.innerHTML='<p style="font-size:10px;color:#64748b">Sin calificaciones</p>';
 }
}
