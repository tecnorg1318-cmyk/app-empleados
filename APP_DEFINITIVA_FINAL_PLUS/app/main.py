...
def login(d: dict):
    try:
        ...
        hp = hash_pass(p)
        # 1. Buscar en admins_db (superadmin, admin)
        if u in admins_db:
            stored = admins_db[u].get("password","")
            if stored == hp or stored == p or stored == hash_pass(p):
                audit_log(u,"login",f"rol {admins_db[u]['rol']}")
                return {"ok": True, "rol": "admin", "subrol": admins_db[u].get("rol","admin"), "usuario": u, "nombre": admins_db[u].get("nombre", u), "empresa_id": admins_db[u].get("empresa_id",""), "token": str(uuid.uuid4())}
        # 2. Buscar en empresa_db info (usuario principal)
        info = empresa_db.get("info", {})
        if info.get("usuario") == u:
            sp = info.get("password","") or info.get("pass","")
            if sp == hp or sp == p:
                return {"ok": True, "rol": "empresa", "usuario": u, "empresa": info.get("empresa",""), "nombre": info.get("nombre_admin",""), "empresa_id": info.get("id","main")}
        # 3. Buscar en sucursales admins (si hay empresas multiples)
        for emp_id, emp_data in empresa_db.items():
            if isinstance(emp_data, dict) and emp_data.get("usuario") == u:
                sp = emp_data.get("password","")
                if sp == hp or sp == p:
                    return {"ok": True, "rol": "empresa", "usuario": u, "empresa_id": emp_id}
...
def registro_empresa(data: dict):
    try:
        ...
        if not all([nombre, usuario, empresa_nombre, direccion, password]):
            raise HTTPException(400, "Faltan campos obligatorios: nombre, usuario, empresa, direccion, password")
        if password != confirm:
        ...
        # Guardar en empresa_db
        if "info" not in empresa_db or not empresa_db.get("info"):
            empresa_db["info"] = empresa_info
        else:
            empresa_db[empresa_id] = empresa_info
            empresa_db["info"] = empresa_info  # Actual como activa

        # Crear admin para esa empresa
        admins_db[usuario] = {
            "password": hp,
            "rol": "admin",
            "nombre": nombre,
        ...
        if not sucursales_db:
            ...
            sucursales_db[suc_id] = {
                "id": suc_id,
                "nombre": "Matriz - " + empresa_nombre,
                "direccion": direccion,
                "empresa_id": empresa_id,
                "telefono": telefono
            }
        ...
        save_db()
        audit_log(usuario, "registro_empresa", f"Empresa {empresa_nombre} creada")

        return {"ok": True, "mensaje": "Empresa registrada correctamente", "empresa_id": empresa_id, "usuario": usuario, "empresa": empresa_nombre}
    except HTTPException:
        raise
    except Exception as e:
        print(f"REGISTRO ERROR: {e}")
        import traceback; traceback.print_exc()
        raise HTTPException(500, f"Error registro: {e}")
