...
def registro_empresa(data: dict):
    try:
        ...
        save_db()
        audit_log(usuario, "registro", f"Empresa {empresa_nombre}")
        ...
        print(f"REG ERROR {e}")
        import traceback; traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Error registro: {e}")
...
def login(d: dict):
    try:
        ...
        if info.get("usuario") == u and info.get("password") in (p, hp):
...
# DUPLICATE registro_empresa REMOVED
