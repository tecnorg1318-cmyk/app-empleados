...
def login(d: dict):
    try:
        ...
        if not u or not p:
            raise HTTPException(status_code=400, detail="Faltan usuario y password")
        hp = hash_pass(p)
        print(f"LOGIN TRY {u} admins={list(admins_db.keys())}")
        if u in admins_db:
            stored = admins_db[u].get("password","")
            if stored == p or stored == hp:
                audit_log(u,"login",f"rol {admins_db[u].get('rol','')}")
        ...
        if info.get("usuario") == u:
            ...
            if stored == p or stored == hp:
                return {"ok": True, "rol": "admin", "usuario": u, "nombre": info.get("nombre_admin",u), "token": str(uuid.uuid4())}
        for eid, emp in empleados_db.items():
            if emp.get("usuario") == u:
                if emp.get("password","") in (p, hp):
                    return {"ok": True, "rol": "empleado", "usuario": u, "empleado_id": eid, "nombre": emp.get("nombre",u), "token": str(uuid.uuid4())}
...
def reset_admin():
    ...
    save_db()
    return {"ok": True, "usuario": "admin", "password": "admin123", "admins": list(admins_db.keys())}
...
def registro_empresa(data: dict):
    try:
        ...
        if password != confirm:
            raise HTTPException(status_code=400, detail="Contrasenas no coinciden")
        if usuario in admins_db:
            raise HTTPException(status_code=400, detail=f"Usuario {usuario} ya existe")
        empresa_id = "emp_" + str(uuid.uuid4())[:8]
        hp = hash_pass(password)
        info = {"id": empresa_id, "nombre_admin": nombre, "usuario": usuario, "empresa": empresa_nombre, "direccion": direccion, "correo": correo, "telefono": telefono, "password": hp, "fecha_registro": get_now_iso()}
        ...
        if not sucursales_db:
            ...
            sucursales_db[sid] = {"id": sid, "nombre": "Matriz - " + empresa_nombre, "direccion": direccion, "empresa_id": empresa_id}
        save_db()
