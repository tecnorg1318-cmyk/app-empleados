...
perfil_fotos_db = {}
asignaciones_flex_db = []  # NUEVO: asignaciones por dia/semana/mes sin quitar nada
...
# === NUEVO: ASIGNACIONES FLEXIBLES POR DIA/SEMANA/MES - AGREGADO SIN QUITAR NADA ===
@app.get("/api/asignaciones-flex")
def get_asig_flex():
    return asignaciones_flex_db

@app.post("/api/asignaciones-flex/dia")
def post_asig_dia(data: dict):
    global asignaciones_flex_db
    # eliminar existente mismo empleado y fecha
    ...
    asignaciones_flex_db.append(data)
    save_db()
    return data
...
def get_hoy_flex(eid: str):
    # logica prioridad dia > semana > mes > base
    from datetime import datetime as dt
    hoy = dt.now()
    ...
    if asig_dia:
        return {"sucursal_id": asig_dia.get("sucursal_id"), "sucursal_nombre": sucursales_db.get(asig_dia.get("sucursal_id"),{}).get("nombre", asig_dia.get("sucursal_id")), "origen":"dia", "fecha":fecha_str, "dia_nombre":dia_nombre.capitalize(), "semana":semana_str, "mes":mes_str, "semana_completa":{}}
    ...
    if asig_sem:
        suc_id = asig_sem.get(dia_nombre,"")
        return {"sucursal_id": suc_id, "sucursal_nombre": sucursales_db.get(suc_id,{}).get("nombre", suc_id), "origen":"semana", "fecha":fecha_str, "dia_nombre":dia_nombre.capitalize(), "semana":semana_str, "mes":mes_str, "semana_completa":{d:asig_sem.get(d,"") for d in dias}}
    asig_mes = next((a for a in asignaciones_flex_db if a.get("tipo")=="mes" and a.get("empleado_id")==eid and a.get("mes")==mes_str), None)
    if asig_mes:
        suc_id = asig_mes.get("sucursal_id","")
        return {"sucursal_id": suc_id, "sucursal_nombre": sucursales_db.get(suc_id,{}).get("nombre", suc_id), "origen":"mes", "fecha":fecha_str, "dia_nombre":dia_nombre.capitalize(), "semana":semana_str, "mes":mes_str, "semana_completa":{}}
    # base
    emp = empleados_db.get(eid,{})
    suc_id = (emp.get("sucursales_ids") or [""])[0] if emp.get("sucursales_ids") else ""
    return {"sucursal_id": suc_id, "sucursal_nombre": sucursales_db.get(suc_id,{}).get("nombre", suc_id), "origen":"base", "fecha":fecha_str, "dia_nombre":dia_nombre.capitalize(), "semana":semana_str, "mes":mes_str, "semana_completa":{}}
...
<div class="card" style="border:1px solid #6366f1"><h3>🗓️ Admin asigna sucursal por DÍA / SEMANA / MES (AGREGADO SIN QUITAR)</h3><p style="font-size:11px;color:var(--muted)">Asigna dónde le toca al empleado cada día. Prioridad: DÍA > SEMANA > MES > Base. El empleado lo ve en Mi Jornada.</p>
...
<label>Empleado</label><select id="flex_emp" class="input"></select>
<div id="flex-box-dia" style="display:none;background:var(--bg);padding:12px;border-radius:12px;margin-top:8px"><label>Fecha</label><input id="flex_fecha" class="input" type="date"><label>Sucursal</label><select id="flex_suc_dia" class="input"></select><button class="btn btn-primary" onclick="guardarFlexDia()">💾 Guardar DÍA</button></div>
<div id="flex-box-semana" style="background:var(--bg);padding:12px;border-radius:12px;margin-top:8px"><label>Semana</label><input id="flex_semana" class="input" type="week"><div style="display:grid;grid-template-columns:1fr 1fr;gap:6px"><div><label>Lun</label><select id="flex_lun" class="input"></select></div><div><label>Mar</label><select id="flex_mar" class="input"></select></div><div><label>Mié</label><select id="flex_mie" class="input"></select></div><div><label>Jue</label><select id="flex_jue" class="input"></select></div><div><label>Vie</label><select id="flex_vie" class="input"></select></div><div><label>Sáb</label><select id="flex_sab" class="input"></select></div><div><label>Dom</label><select id="flex_dom" class="input"></select></div></div><button class="btn btn-primary" onclick="guardarFlexSemana()">💾 Guardar SEMANA</button></div>
<div id="flex-box-mes" style="display:none;background:var(--bg);padding:12px;border-radius:12px;margin-top:8px"><label>Mes</label><input id="flex_mes" class="input" type="month"><label>Sucursal mes</label><select id="flex_suc_mes" class="input"></select><button class="btn btn-primary" onclick="guardarFlexMes()">💾 Guardar MES</button></div>
<p id="flex-msg" style="font-size:11px;color:#10b981;margin-top:8px"></p>
</div>
<div class="card"><h3>📋 Asignaciones actuales</h3><div style="display:flex;gap:6px;flex-wrap:wrap"><button class="btn btn-dark" style="width:auto" onclick="cargarFlexAsignaciones('')">TODO</button><button class="btn btn-dark" style="width:auto" onclick="cargarFlexAsignaciones('dia')">DÍA</button><button class="btn btn-dark" style="width:auto" onclick="cargarFlexAsignaciones('semana')">SEMANA</button><button class="btn btn-dark" style="width:auto" onclick="cargarFlexAsignaciones('mes')">MES</button></div><div id="flex-lista" style="margin-top:12px;max-height:600px;overflow:auto"></div></div>
</div>
...
// === AGREGADO SIN QUITAR: ASIGNACIONES FLEX DIA/SEMANA/MES ===
...
async function cargarFlexEmpsSucs(){
 try{
  ...
  const empOpts=emps.map(e=>`<option value="${e.id}">${e.id} - ${e.nombre}</option>`).join('');
  const sucOpts=sucs.map(s=>`<option value="${s.id}">${s.nombre}</option>`).join('')+'<option value="">Descanso</option>';
  const sucOpts2=sucs.map(s=>`<option value="${s.id}">${s.nombre}</option>`).join('');
  ['flex_emp'].forEach(id=>{const el=document.getElementById(id); if(el) el.innerHTML=empOpts;});
  ['flex_suc_dia','flex_suc_mes'].forEach(id=>{const el=document.getElementById(id); if(el) el.innerHTML=sucOpts2;});
  ['flex_lun','flex_mar','flex_mie','flex_jue','flex_vie','flex_sab','flex_dom'].forEach(id=>{const el=document.getElementById(id); if(el) el.innerHTML=sucOpts;});
 }catch(e){console.log('flex load error',e)}
}
async function guardarFlexDia(){
 const d={empleado_id:document.getElementById('flex_emp').value, fecha:document.getElementById('flex_fecha').value, sucursal_id:document.getElementById('flex_suc_dia').value};
 if(!d.empleado_id||!d.fecha||!d.sucursal_id) return alert('Falta empleado, fecha, sucursal');
 const r=await api('/api/asignaciones-flex/dia','POST',d);
 document.getElementById('flex-msg').innerText='✅ DÍA guardado '+d.empleado_id+' → '+d.sucursal_id;
 cargarFlexAsignaciones('dia');
}
async function guardarFlexSemana(){
 const d={empleado_id:document.getElementById('flex_emp').value, semana:document.getElementById('flex_semana').value, lunes:document.getElementById('flex_lun').value, martes:document.getElementById('flex_mar').value, miercoles:document.getElementById('flex_mie').value, jueves:document.getElementById('flex_jue').value, viernes:document.getElementById('flex_vie').value, sabado:document.getElementById('flex_sab').value, domingo:document.getElementById('flex_dom').value};
 if(!d.empleado_id||!d.semana) return alert('Falta empleado y semana');
 const r=await api('/api/asignaciones-flex/semana','POST',d);
 document.getElementById('flex-msg').innerText='✅ SEMANA guardada '+d.semana;
 cargarFlexAsignaciones('semana');
}
async function guardarFlexMes(){
 const d={empleado_id:document.getElementById('flex_emp').value, mes:document.getElementById('flex_mes').value, sucursal_id:document.getElementById('flex_suc_mes').value};
 if(!d.empleado_id||!d.mes||!d.sucursal_id) return alert('Falta empleado, mes, sucursal');
 const r=await api('/api/asignaciones-flex/mes','POST',d);
 document.getElementById('flex-msg').innerText='✅ MES guardado '+d.mes;
 cargarFlexAsignaciones('mes');
}
async function cargarFlexAsignaciones(filtro){
 let url='/api/asignaciones-flex';
 const list=await api(url);
 let filtered=list;
 if(filtro) filtered=list.filter(a=>a.tipo===filtro);
 document.getElementById('flex-lista').innerHTML=filtered.map(a=>{
  let btn=`<button onclick="eliminarFlex('${a.id}')" style="background:#ef444422;color:#f87171;border:none;padding:4px 8px;border-radius:8px;cursor:pointer;font-size:11px">🗑️ Eliminar</button>`;
  if(a.tipo==='dia') return `<div style="background:var(--bg);padding:8px;border-radius:8px;margin-top:6px;border-left:4px solid #38bdf8;display:flex;justify-content:space-between"><span>📅 <b>DÍA</b> ${a.fecha} - Emp ${a.empleado_id} → ${a.sucursal_id}</span>${btn}</div>`;
  if(a.tipo==='semana') return `<div style="background:var(--bg);padding:8px;border-radius:8px;margin-top:6px;border-left:4px solid #a78bfa"><div style="display:flex;justify-content:space-between"><span>🗓️ <b>SEMANA</b> ${a.semana} - ${a.empleado_id}</span>${btn}</div><div style="font-size:10px;margin-top:4px">L:${a.lunes||'-'} M:${a.martes||'-'} X:${a.miercoles||'-'} J:${a.jueves||'-'} V:${a.viernes||'-'} S:${a.sabado||'-'} D:${a.domingo||'-'}</div></div>`;
  if(a.tipo==='mes') return `<div style="background:var(--bg);padding:8px;border-radius:8px;margin-top:6px;border-left:4px solid #fb923c;display:flex;justify-content:space-between"><span>📆 <b>MES</b> ${a.mes} - ${a.empleado_id} → ${a.sucursal_id}</span>${btn}</div>`;
 }).join('') || '<p style="font-size:11px;color:var(--muted)">Sin asignaciones</p>';
}
async function eliminarFlex(id){
 if(!confirm('¿Eliminar asignación?')) return;
 await api('/api/asignaciones-flex/'+id,'DELETE');
 cargarFlexAsignaciones('');
}
// === AGREGADO SIN QUITAR: HISTORIAL CALIFICACIONES ABAJO DE JORNADA Y RETARDOS ===
async function cargarMiCalificacionesHistorial(){
 try{
  ...
  // Buscar contenedores en empleado jornada
  ...
  if(jornadaDiv && !document.getElementById('emp-calif-below-jornada')){
   ...
   box.id='emp-calif-below-jornada';
   box.className='card';
  ...
  if(rankingDiv && !document.getElementById('emp-calif-below-retardos')){
   ...
   box2.id='emp-calif-below-retardos';
   box2.className='card';
   box2.style.border='1px solid #f59e0b';
   box2.innerHTML=`<h3>⏱️ Mis Retardos + ⭐ Calificaciones</h3><div id="emp-retardos-detalle"></div><div id="emp-calif-retardos-lista" style="margin-top:12px"></div>`;
   rankingDiv.appendChild(box2);
  }
  // Render
  const listaJ=document.getElementById('emp-calif-jornada-lista');
  const promJ=document.getElementById('emp-calif-jornada-prom');
  const listaR=document.getElementById('emp-calif-retardos-lista');
  const retDetalle=document.getElementById('emp-retardos-detalle');
  if(evals2.length>0){
   evals2.sort((a,b)=>new Date(b.fecha)-new Date(a.fecha));
   const html=evals2.map((e,i)=>`<div style="background:#0f172a;padding:8px;border-radius:8px;margin-top:6px;border-left:3px solid ${e.total>=80?'#10b981':e.total>=60?'#f59e0b':'#ef4444'}"><div style="display:flex;justify-content:space-between"><span style="font-size:11px;color:#94a3b8">📅 ${e.fecha||e.created_at||''}</span><span style="font-weight:800;color:${e.total>=80?'#34d399':e.total>=60?'#fbbf24':'#f87171'}">⭐ ${e.total||e.calificacion||0}/100</span></div><p style="font-size:11px;margin-top:4px">${e.comentario||JSON.stringify(e.calificaciones||{}).slice(0,100)}</p></div>`).join('');
   if(listaJ) listaJ.innerHTML=html;
   if(listaR) listaR.innerHTML=html;
   const prom=(evals2.reduce((s,x)=>s+(x.total||x.calificacion||0),0)/evals2.length).toFixed(1);
   if(promJ) promJ.innerHTML=`<p style="font-size:12px">Promedio: <b style="font-size:18px;color:#a78bfa">⭐ ${prom}/100</b> de ${evals2.length} evaluaciones</p>`;
  } else {
   if(listaJ) listaJ.innerHTML='<p style="font-size:11px;color:#64748b">Sin calificaciones aún</p>';
   if(listaR) listaR.innerHTML='<p style="font-size:11px;color:#64748b">Sin calificaciones</p>';
  }
  // Retardos
  try{
   const retardos=await api('/empleado/'+USER_ID+'/retardos-mes');
   if(retDetalle) retDetalle.innerHTML=`<p style="font-size:11px">Faltas: <b>${retardos.faltas||0}</b> | Retardos: <b>${retardos.retardos||0}</b></p><div style="max-height:150px;overflow:auto;margin-top:6px">${(retardos.detalles||[]).slice(0,8).map(r=>`<div style="font-size:10px;padding:4px;border-bottom:1px solid #1e293b">${r.fecha_dia} ${r.entrada||''} ${r.retardo_entrada>0?'🔴 +'+r.retardo_entrada+'min':'🟢'}</div>`).join('')}</div>`;
  }catch(e){}
 }catch(e){console.log('calif hist error',e)}
}
// Hook a cargarEmpleadoPro
const _origCargarEmpleadoPro = cargarEmpleadoPro;
cargarEmpleadoPro = async function(){ await _origCargarEmpleadoPro(); await cargarFlexEmpsSucs(); await cargarMiCalificacionesHistorial(); await cargarFlexAsignaciones(''); }
