...
@app.get("/api/panico")
def get_panico(): return panico_db[-100:]
@app.post("/api/panico")
def post_panico(data: dict):
    data["id"]=str(uuid.uuid4())[:8]; data["created_at"]=datetime.now().isoformat(); data["fecha"]=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    panico_db.append(data); save_db(); return data
...
async function activarPanico(){
 if(!confirm('🚨 ¿Activar BOTÓN DE PÁNICO? Esto alertará al admin con tu ubicación GPS y sucursal.')) return;
 try{
  const pos = await new Promise((res,rej)=> navigator.geolocation.getCurrentPosition(res,rej,{enableHighAccuracy:true, timeout:10000}));
  const lat=pos.coords.latitude; const lng=pos.coords.longitude;
  const data={empleado_id:USER_ID, nombre: localStorage.getItem('nombre')||USER_ID, lat, lng, sucursal: document.getElementById('emp-sucursal-hoy')?.innerText||'', mensaje:'🚨 PÁNICO SOS', gps:`https://maps.google.com/?q=${lat},${lng}`};
  await api('/api/panico','POST',data);
  alert('🚨 ¡ALERTA ENVIADA! Admin ya recibió tu ubicación. Mantén la calma.');
  // WhatsApp opcional
  const telAdmin = (await api('/api/config-admin')).telefono_admin || '';
  if(telAdmin){ window.open(`https://wa.me/${telAdmin}?text=${encodeURIComponent('🚨 PÁNICO SOS de '+data.nombre+' - '+data.gps+' - Sucursal: '+data.sucursal)}`,'_blank'); }
 }catch(e){
  // Si falla GPS, envia sin GPS
  await api('/api/panico','POST',{empleado_id:USER_ID, nombre: localStorage.getItem('nombre')||USER_ID, mensaje:'🚨 PÁNICO SOS sin GPS', lat:0,lng:0});
  alert('🚨 Alerta enviada sin GPS (no diste permiso de ubicación). Admin notificado.');
 }
}
async function cargarPanico(){
 const list=await api('/api/panico');
 const cont=document.getElementById('panico-lista');
 const badge=document.getElementById('panico-badge');
 if(badge){ if(list.length>0){ badge.style.display='inline'; badge.innerText=list.length; } else badge.style.display='none'; }
 if(!cont) return;
 cont.innerHTML=list.slice(-50).reverse().map(p=>`<div style="background:white;border:2px solid #ef4444;border-radius:12px;padding:12px;margin-top:8px"><div style="display:flex;justify-content:space-between"><span style="font-weight:800;color:#dc2626">🚨 ${p.nombre||p.empleado_id} - ${p.fecha||p.created_at||''}</span><span style="font-size:10px">${p.sucursal||''}</span></div><p style="font-size:12px;margin-top:4px">${p.mensaje||''}</p>${p.lat?`<a href="https://maps.google.com/?q=${p.lat},${p.lng}" target="_blank" style="display:inline-block;background:#ef4444;color:white;padding:6px 10px;border-radius:8px;text-decoration:none;font-size:11px;margin-top:6px">📍 Ver ubicación GPS</a>`:''} <button onclick="atenderPanico('${p.id}')" style="background:#10b981;color:white;border:none;padding:6px 10px;border-radius:8px;font-size:11px;margin-left:6px">✅ Atendido</button></div>`).join('')||'<p style="color:#64748b">Sin alertas de pánico</p>';
 if(list.length>0 && ROL!=='empleado'){
   // Sonido alerta
   try{ const audio=new Audio('https://cdn.pixabay.com/download/audio/2021/08/04/audio_0625c1539c.mp3'); audio.volume=0.5; audio.play().catch(()=>{}); }catch(e){}
 }
}
async function atenderPanico(id){ if(!confirm('¿Marcar como atendido?')) return; await api('/api/panico/'+id,'PUT',{atendido:true}); cargarPanico(); }
async function limpiarPanico(){ if(!confirm('¿Limpiar todas las alertas?')) return; const list=await api('/api/panico'); for(let p of list){ try{ await api('/api/panico/'+p.id,'DELETE'); }catch(e){} } cargarPanico(); }

// Inyectar botón pánico abajo de retardos y horas en empleado
async function inyectarBotonPanicoEmpleado(){
 const tryInject=()=>{
  const retardosCard=document.getElementById('emp-retardos-detalle')?.closest('.card') || document.getElementById('tab-emp-jornada');
  const horasCard=document.getElementById('emp-calif-below-jornada') || document.getElementById('tab-emp-jornada');
  if(!retardosCard && !horasCard) return false;
  if(document.getElementById('btn-panico-empleado')) return true;
  const cont=retardosCard?.parentNode || document.getElementById('tab-emp-jornada') || document.getElementById('empleado-area');
  if(!cont) return false;
  const box=document.createElement('div'); box.id='panic-container-empleado'; box.className='card'; box.style.border='3px solid #ef4444'; box.style.background='linear-gradient(135deg,#fef2f2,#fee2e2)'; box.style.textAlign='center';
  box.innerHTML=`<h3 style="color:#dc2626">🚨 Botón de Pánico SOS</h3><p style="font-size:11px;color:#7f1d1d">Si tienes emergencia, presiona abajo. Admin recibirá tu ubicación GPS, sucursal y hora al instante.</p><button id="btn-panico-empleado" onclick="activarPanico()" style="background:linear-gradient(135deg,#ef4444,#dc2626);color:white;border:none;padding:16px 24px;border-radius:16px;font-weight:800;font-size:16px;width:100%;margin-top:12px;box-shadow:0 8px 20px rgba(239,68,68,.4);cursor:pointer">🚨 ACTIVAR PÁNICO SOS</button><p style="font-size:9px;color:#991b1b;margin-top:8px">Abajo de retardos y horas - Admin lo recibe en pestaña Pánico SOS</p>`;
...
async function cargarDashboardPro(){
 try{
  const asist=await api('/asistencias'); const evals=await api('/evaluaciones');
  const hoy=new Date().toISOString().slice(0,10);
  const hoyAsist=Array.isArray(asist)? asist.filter(a=>a.fecha===hoy || a.fecha?.includes(hoy)) : [];
  const dash=document.getElementById('dashboard-pro');
  if(dash){
    dash.innerHTML=`<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px"><div style="background:#6366f122;padding:12px;border-radius:12px;text-align:center"><b style="font-size:20px">${hoyAsist.length}</b><br><small>Asist hoy</small></div><div style="background:#ef444422;padding:12px;border-radius:12px;text-align:center"><b style="font-size:20px">${(await api('/api/panico')).length}</b><br><small>Alertas pánico</small></div><div style="background:#f59e0b22;padding:12px;border-radius:12px;text-align:center"><b style="font-size:20px">${evals.length}</b><br><small>Evaluaciones</small></div><div style="background:#10b98122;padding:12px;border-radius:12px;text-align:center"><b style="font-size:20px">${Object.keys(await api('/sucursales')).length}</b><br><small>Sucursales</small></div></div>`;
  }
 }catch(e){}
}
async function mensajeMasivo(){
 const suc=document.getElementById('masivo_suc').value; const msg=document.getElementById('masivo_msg').value;
 if(!suc||!msg) return alert('Sucursal y mensaje');
 const emps=await api('/empleados'); const list=Array.isArray(emps)?emps:Object.values(emps);
 const filtrados=list.filter(e=> (e.sucursales_ids||[]).includes(suc) || e.sucursal_id===suc);
 if(!filtrados.length) return alert('No hay empleados en esa sucursal');
 const texto=encodeURIComponent(msg);
 // Genera links wa.me
 const links=filtrados.map(e=>`https://wa.me/${e.telefono||''}?text=${texto}`).join('\n');
 alert(`📲 Mensaje masivo listo para ${filtrados.length} empleados. Se abrirá WhatsApp.` );
 if(filtrados[0].telefono) window.open(`https://wa.me/${filtrados[0].telefono}?text=${texto}`,'_blank');
}

const _origCargarTodo_PANIC=cargarTodo;
cargarTodo=async function(){ await _origCargarTodo_PANIC(); try{ await cargarPanico(); await cargarDashboardPro(); }catch(e){} };

const _origCargarEmpleadoPro_PANIC=cargarEmpleadoPro;
cargarEmpleadoPro=async function(){ await _origCargarEmpleadoPro_PANIC(); try{ await inyectarBotonPanicoEmpleado(); }catch(e){} };
