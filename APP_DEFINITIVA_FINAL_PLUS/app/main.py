...
def post_manual(data: dict):
    data["id"]=str(uuid.uuid4())[:8]; data["created_at"]=datetime.now().isoformat(); manuales_db.append(data); save_db(); return data
...
def post_multas(data: dict):
    data["id"]=str(uuid.uuid4())[:8]; data["created_at"]=datetime.now().isoformat(); multas_db.append(data); save_db(); return data
@app.get("/api/avisos")
...
def post_aviso(data: dict):
    data["id"]=str(uuid.uuid4())[:8]; data["created_at"]=datetime.now().isoformat()
...
def post_buzon(data: dict):
    data["id"]=str(uuid.uuid4())[:8]; data["created_at"]=datetime.now().isoformat(); data["anonimo"]=True; buzon_db.append(data); save_db(); return data
@app.get("/api/capacitaciones")
def get_caps(): return capacitaciones_db
@app.post("/api/capacitaciones")
def post_cap(data: dict):
    data["id"]=str(uuid.uuid4())[:8]; data["created_at"]=datetime.now().isoformat(); data["vistas"]=[]; capacitaciones_db.append(data); save_db(); return data
...
CLOCK RD es un sistema de control de asistencia, geolocalización, gestión de personal, evaluación, nómina y comunicación interna para empresas. Su uso es exclusivo para empleados y administradores autorizados por la empresa contratante.
