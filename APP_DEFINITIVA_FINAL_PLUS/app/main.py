...
def get_hoy_str():
    return get_now_mexico().strftime("%Y-%m-%d")

def get_hora_str():
    return get_now_mexico().strftime("%H:%M:%S")
...
if HAS_SQLALCHEMY:
    try:
        ...
        print(f"Base de datos lista: {str(DATABASE_URL)[:20] if DATABASE_URL else DB_SQLITE}")
...
manuales_db = []  # Manual empresa PDF
multas_db = []  # Multas por retardo
avisos_db = []  # Muro avisos
buzon_db = []  # Buzón anónimo
capacitaciones_db = []  # Videos capacitación
puntos_db = {}  # Gamificación
reporte_auto_db = {"activo": True, "hora": "21:00"}  # Reporte auto
mapa_vivo_db = []  # Mapa vivo cache
...
def load_db():
    ...
    if HAS_SQLALCHEMY and DATABASE_URL:
        try:
            ...
            s = Session()
            row = s.query(AppDataDB).filter(AppDataDB.key=="main").first()
            if row and row.value:
                data=json.loads(row.value)
                sucursales_db=data.get("sucursales",{})
                empleados_db=data.get("empleados",{"EMPDEMO": {"id":"EMPDEMO","nombre":"Empleado Demo","puesto":"Demo","rol":"empleado","password":hash_pass("demo"),"sucursales_ids":[],"horario":{},"activo":True,"tiempo_comida":120,"telefono":"5210000000000","sueldo_hora":50,"foto":""},"EMP0001": {"id":"EMP0001","nombre":"Empleado Prueba","puesto":"Botarga","rol":"empleado","password":hash_pass("0001"),"sucursales_ids":[],"horario":{},"activo":True,"tiempo_comida":120,"telefono":"5210000000000","sueldo_hora":50,"foto":""}})
                evaluaciones_db=data.get("evaluaciones",[]); asistencias_db=data.get("asistencias",[]); alertas_db=data.get("alertas",[]); gps_logs_db=data.get("gps_logs",[]); vacaciones_db=data.get("vacaciones",[]); justificantes_db=data.get("justificantes",[]); audit_db=data.get("audit",[]); chat_db=data.get("chat",[]); panico_db=data.get("panico",[]); reportes_volanteo_db=data.get("reportes_volanteo",[]); empresa_db=data.get("empresa",{}); verificaciones_db=data.get("verificaciones",{}); permisos_db=data.get("permisos",permisos_db); bonos_db=data.get("bonos",{}); metas_db=data.get("metas",{}); nomina_db=data.get("nomina",{}); notificaciones_db=data.get("notificaciones",[]); turnos_rotativos_db=data.get("turnos_rotativos",{}); config_admin_db=data.get("config_admin",config_admin_db); perfil_fotos_db=data.get("perfil_fotos",{}); asignaciones_flex_db=data.get("asignaciones_flex",[]); tareas_sucursal_db=data.get("tareas_sucursal",[]); horas_extra_db=data.get("horas_extra",[]); fotos_checkin_db=data.get("fotos_checkin",{}); manuales_db=data.get("manuales",[]); 
                # Cargar admins creados por registro empresa
                if "admins" in data:
                    admins_db.update(data.get("admins",{}))
                multas_db=data.get("multas",[]); avisos_db=data.get("avisos",[]); buzon_db=data.get("buzon",[]); capacitaciones_db=data.get("capacitaciones",[]); puntos_db=data.get("puntos",{}); reporte_auto_db=data.get("reporte_auto",{"activo": True, "hora": "21:00"})
                s.close(); print(" Cargado desde Neon"); return
            s.close()
        except Exception as e:
            print(f"Error load Neon: {e}")
...
def save_db():
    # FIX V4.1 PERSISTENCIA DEFINITIVA - No borra nunca
    data_full = {"sucursales":sucursales_db,"empleados":empleados_db,"evaluaciones":evaluaciones_db,"asistencias":asistencias_db,"alertas":alertas_db,"gps_logs":gps_logs_db,"vacaciones":vacaciones_db,"justificantes":justificantes_db,"audit":audit_db,"chat":chat_db,"panico":panico_db,"reportes_volanteo":reportes_volanteo_db,"empresa":empresa_db,"verificaciones":verificaciones_db,"permisos":permisos_db,"bonos":bonos_db,"metas":metas_db,"nomina":nomina_db,"notificaciones":notificaciones_db,"turnos_rotativos":turnos_rotativos_db,"config_admin":config_admin_db,"perfil_fotos":perfil_fotos_db, "asignaciones_flex":asignaciones_flex_db, "tareas_sucursal":tareas_sucursal_db, "horas_extra":horas_extra_db, "fotos_checkin":fotos_checkin_db, "manuales":manuales_db, "admins":admins_db, "multas":multas_db, "avisos":avisos_db, "buzon":buzon_db, "capacitaciones":capacitaciones_db, "puntos":puntos_db, "reporte_auto":reporte_auto_db}
    # Guardar siempre en Neon si existe
    if HAS_SQLALCHEMY and DATABASE_URL:
        for intento in range(3):
            try:
                ...
                s = Session()
                row = s.query(AppDataDB).filter(AppDataDB.key=="main").first()
                js = json.dumps(data_full, ensure_ascii=False)
                if row:
                    row.value = js
                else:
                    s.add(AppDataDB(key="main", value=js))
                s.commit()
                s.close()
                print(f" Guardado en Neon Postgres intento {intento+1} - empresas:{len(empresa_db)} empleados:{len(empleados_db)}")
                break
            except Exception as e:
                print(f" Error guardando Postgres intento {intento+1}: {e}")
                try:
                    s.rollback(); s.close()
                except:
                    pass
                import time; time.sleep(1)
    # Backup local tambien
    try:
        with open(DB_FILE,'w', encoding='utf-8') as f: json.dump(data_full, f, ensure_ascii=False, indent=2)
    except Exception as e:
        print(e)
...
def calcular_distancia_m(lat1, lon1, lat2, lon2):
    try:
        ...
        return 999999
def detectar_mock_gps(data):
    motivos=[]
    try:
        acc=float(data.get('accuracy',0) or 0)
        if acc==0 or acc>2000:
            motivos.append(f"Accuracy sospechosa {acc}")
        if data.get('mock')==True or data.get('is_mock'):
            motivos.append("Mock GPS")
        lat=float(data.get('lat',0)); lon=float(data.get('lon',0))
        if abs(lat)<0.001 and abs(lon)<0.001:
            motivos.append("0,0")
    except Exception as e:
        print(f"antitrampa err {e}")
    return motivos
DEFAULT_GEOCERCA_RADIO=150
    ...
    for eid, emp in empleados_db.items():
        ...
        for i in range(1,4):
            ...
            if not asist or not asist.get("entrada"):
                faltas+=1
        if faltas>=3:
            alertas.append({"empleado_id":eid,"nombre":emp.get("nombre"),"telefono":emp.get("telefono"),"faltas":faltas,"mensaje":f" {emp.get('nombre')} lleva {faltas} faltas seguidas"})
    return alertas
...
@app.get("/api/avisos")
def get_avisos(): return avisos_db
@app.get("/api/avisos/{eid}")
def get_avisos_emp(eid: str):
    # Devuelve avisos para todos o específicos
    res=[]
    for a in avisos_db:
        para=a.get("para","todos")
        if para=="todos" or para=="all" or eid in (para if isinstance(para,list) else [para]) or eid=="admin":
            res.append(a)
    return res
@app.post("/api/avisos")
def post_aviso(data: dict):
    data["id"]=str(uuid.uuid4())[:8]; data["created_at"]=get_now_iso()
    # para puede ser "todos" o lista de ids
    if "para" not in data: data["para"]="todos"
    avisos_db.append(data); save_db(); return data
@app.delete("/api/avisos/{aid}")
def del_aviso(aid: str):
    global avisos_db; avisos_db[:] = [a for a in avisos_db if a.get("id")!=aid]; save_db(); return {"ok":True}
@app.get("/api/buzon")
def get_buzon(): return buzon_db
@app.post("/api/buzon")
def post_buzon(data: dict):
    data["id"]=str(uuid.uuid4())[:8]; data["created_at"]=get_now_iso(); data["anonimo"]=True; buzon_db.append(data); save_db(); return data
@app.get("/api/capacitaciones")
def get_caps(): return capacitaciones_db
@app.post("/api/capacitaciones")
def post_cap(data: dict):
    data["id"]=str(uuid.uuid4())[:8]; data["created_at"]=get_now_iso(); data["vistas"]=[]; capacitaciones_db.append(data); save_db(); return data
@app.post("/api/capacitaciones/{cid}/visto")
def cap_visto(cid: str, data: dict):
    eid=data.get("empleado_id")
    for c in capacitaciones_db:
        if c.get("id")==cid and eid not in c.get("vistas",[]):
            c["vistas"].append(eid); save_db()
    return {"ok":True}
@app.get("/api/puntos")
def get_puntos(): return puntos_db
@app.post("/api/puntos/sumar")
def sumar_puntos(data: dict):
    eid=data.get("empleado_id"); pts=data.get("puntos",0)
    if eid not in puntos_db: puntos_db[eid]={"puntos":0,"medallas":[]}
    puntos_db[eid]["puntos"]=puntos_db[eid].get("puntos",0)+pts
    # Medallas
    p=puntos_db[eid]["puntos"]
    medallas=[]
    if p>=100: medallas.append("🥉 Bronce")
    if p>=300: medallas.append("🥈 Plata")
    if p>=500: medallas.append("🥇 Oro")
    if p>=1000: medallas.append("🏆 Diamante")
    puntos_db[eid]["medallas"]=medallas
    save_db()
    return puntos_db[eid]
@app.get("/api/reporte-auto")
def get_reporte_auto(): return reporte_auto_db
@app.post("/api/reporte-auto")
def set_reporte_auto(data: dict):
    global reporte_auto_db; reporte_auto_db.update(data); save_db(); return reporte_auto_db
@app.put("/api/vacaciones/{vid}/editar")
def editar_vac(vid: str, data: dict): 
...
def db_status():
    ...
    return {
        ...
        "mensaje": "Base de datos conectada " if tipo!="JSON" else "Usando JSON temporal  - Crea PostgreSQL en Render"
    }
...
@app.get("/api/sucursales/geocerca")
def get_geocerca():
    res=[]
    for sid, s in sucursales_db.items():
        res.append({"id": sid, "nombre": s.get("nombre",""), "lat": s.get("lat"), "lon": s.get("lon"), "radio": s.get("radio_geocerca", DEFAULT_GEOCERCA_RADIO)})
    return res
@app.post("/api/sucursales/{sid}/geocerca")
def set_geocerca(sid: str, radio: int = 150, lat: float = None, lon: float = None):
    if sid not in sucursales_db:
        raise HTTPException(404, "Sucursal no existe")
    sucursales_db[sid]["radio_geocerca"]=int(radio)
    if lat is not None: sucursales_db[sid]["lat"]=lat
    if lon is not None: sucursales_db[sid]["lon"]=lon
    save_db()
    return {"ok": True, "radio": radio, "id": sid}
@app.post("/api/check-anti-trampa")
def check_anti_trampa(data: dict):
    motivos=detectar_mock_gps(data)
    return {"sospechoso": len(motivos)>0, "motivos": motivos}
@app.post("/api/checkin-validado")
def checkin_validado(data: dict):
    sid=data.get("sucursal_id"); lat=data.get("lat"); lon=data.get("lon")
    motivos=detectar_mock_gps(data)
    if motivos:
        return {"ok": False, "error": "ANTI-TRAMPA", "motivos": motivos}
    if sid and sid in sucursales_db and lat and lon:
        suc=sucursales_db[sid]
        s_lat=suc.get("lat"); s_lon=suc.get("lon")
        if s_lat and s_lon:
            dist=calcular_distancia_m(s_lat, s_lon, lat, lon)
            radio=suc.get("radio_geocerca", DEFAULT_GEOCERCA_RADIO)
            if dist > radio:
                return {"ok": False, "error": "FUERA_DE_RANGO", "distancia": int(dist), "radio": radio}
    return {"ok": True}
...
async function guardarFlexDia(){
 ...
 if(!emp) return alert(' Selecciona un empleado primero - si no ves, dale a Recargar empleados y sucursales');
...
async function guardarFlexSemana(){
 const emp=document.getElementById('flex_emp').value;
 if(!emp) return alert(' Selecciona empleado primero');
