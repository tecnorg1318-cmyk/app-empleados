...
class VisitaCreate(BaseModel):
    empleado_id: str; sucursal_id: str; sucursal_nombre: str=""; notas: str=""; empresa_id: str="DEFAULT"
...
<div class="card"><h3>📊 Estatus</h3><canvas id="chartEstatus" height="180"></canvas></div>
<div class="card"><h3>⏰ Retardos Top 5</h3><canvas id="chartRet" height="180"></canvas></div>
</div>
<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:12px">
...
<div class="card"><h3>🔔 Notificaciones</h3><div id="dash_notis" style="font-size:11px;margin-top:8px"></div>
<div style="margin-top:12px"><h4 style="font-size:11px">🔑 Cambiar Pass Admin</h4><label>Nueva pass admin</label><input id="new_admin_pass" class="input" type="password"><button class="btn btn-o" onclick="cambiarPassAdmin()">Cambiar pass admin</button><p id="msg_admin_pass" style="font-size:10px;color:#34d399"></p></div>
</div>
</div>
</div>
</div>
...
<div class="card"><h3>Lista empleados (con foto, estatus, faltas) - AHORA con ✏️ Editar / 🗑️ Eliminar + Buscador</h3><label>Buscar</label><input id="busc_emp" class="input" placeholder="Buscar por nombre, ID o puesto" onkeyup="filtrarEmps()"><table class="table"><thead><tr><th>Foto</th><th>ID/Nombre</th><th>Estatus</th><th>Faltas/Ret</th><th>Eval</th><th>Acción</th></tr></thead><tbody id="tab_emp"></tbody></table></div>
</div>
...
<div class="card"><h3>🏪 Sucursal - Ahora con editar/eliminar</h3><label>ID</label><input id="suc_id" class="input"><label>Nombre</label><input id="suc_nom" class="input"><button class="btn btn-p" onclick="crearSuc()">Crear / Guardar</button><button class="btn btn-d" onclick="limpiarSuc()">Limpiar</button></div>
<div class="card"><h3>Sucursales - Click ✏️ para editar</h3><table class="table"><thead><tr><th>ID</th><th>Nombre</th><th>Acción</th></tr></thead><tbody id="tab_suc"></tbody></table></div>
</div>

<div id="sec-mi" style="display:none">
<div class="grid">
<div class="card" style="border:2px solid #10b981"><h3>📱 Login Empleado - Mi Día (tu original + mejoras)</h3>
...
<div class="card"><h3>Mi info completa + NUEVO sueldo e historial</h3><div id="mi_info"></div><button class="btn btn-g" onclick="checkIn()" style="margin-top:10px">✅ Check-In</button><p style="font-size:10px;color:#64748b;margin-top:6px">Si llegas después de tu horario entrada, se cuenta retardo automático. Si no haces check, al final del día es falta (lo ve admin).</p></div>
</div>
</div>

<div id="sec-hist" style="display:none">
<div class="card" style="border:1px dashed #f59e0b"><h3>🔒 Historial + Faltas/Retardos (SOLO ADMIN) + Reporte NUEVO + Filtros</h3>
<div style="display:flex;gap:6px;flex-wrap:wrap;margin:10px 0">
<button class="btn btn-d" style="width:auto" onclick="cargarHist()">🔄 Actualizar historial</button>
<button class="btn btn-o" style="width:auto" onclick="descargarReporte()">📥 Descargar Reporte Nómina CSV</button>
</div>
<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:8px"><div><label>Filtrar por empleado/sucursal</label><input id="filt_hist_emp" class="input" placeholder="EMP001 o Matriz" onkeyup="filtrarHist()"></div><div><label>Filtrar por fecha</label><input id="filt_hist_fecha" class="input" type="date" onchange="filtrarHist()"></div></div>
...
<div id="reporte_box" style="margin-top:12px;background:#0f172a;padding:10px;border-radius:10px;font-size:11px"><h4>💰 Nómina calculada (sueldo - faltas - retardos)</h4></div>
</div>
</div>
...
function filtrarEmps(){
 let q=document.getElementById('busc_emp').value.toLowerCase();
 let emps=window._empsCache||[];
 let filtrados=emps.filter(e=> (e.nombre+e.id+e.puesto).toLowerCase().includes(q));
 document.getElementById('tab_emp').innerHTML=filtrados.map(e=>{
   let estCls='st-activo'; if(e.estatus==='baja') estCls='st-baja'; if(e.estatus==='vacaciones') estCls='st-vac'; if(e.estatus==='incapacidad') estCls='st-inc';
   let foto=e.foto_url?`<img src="${e.foto_url}" class="img-emp">`:`<div class="img-emp" style="display:flex;align-items:center;justify-content:center;font-size:10px">${e.nombre.charAt(0)}</div>`;
   return `<tr><td>${foto}</td><td><b>${e.id}</b><br>${e.nombre}</td><td><span class="badge ${estCls}">${e.estatus}</span></td><td>F:${e.faltas} R:${e.retardos}</td><td>⭐ ${e.promedio_eval||0}</td><td><button onclick="editarEmp('${e.id}')" class="btn btn-d" style="padding:4px;width:auto">✏️</button> <button onclick="eliminarEmp('${e.id}')" class="btn btn-d" style="padding:4px;width:auto;color:#f87171">🗑️</button></td></tr>`;
 }).join('');
}
async function cargarSucs(){
 const sucs=await api('/sucursales?empresa_id='+EMPRESA_ID);
 window._sucsCache=sucs;
 document.getElementById('tab_suc').innerHTML=sucs.map(s=>`<tr><td>${s.id}</td><td>${s.nombre}</td><td><button onclick="editarSuc('${s.id}')" class="btn btn-d" style="padding:4px;width:auto">✏️</button> <button onclick="eliminarSuc('${s.id}')" class="btn btn-d" style="padding:4px;width:auto;color:#f87171">🗑️</button></td></tr>`).join('');
...
function editarSuc(id){ let s=(window._sucsCache||[]).find(x=>x.id===id); if(!s) return; document.getElementById('suc_id').value=s.id; document.getElementById('suc_nom').value=s.nombre; }
async function eliminarSuc(id){ if(!confirm('Eliminar sucursal '+id+'?')) return; await api('/sucursales/'+id+'?empresa_id='+EMPRESA_ID,'DELETE'); cargarSucs(); }
function limpiarSuc(){ document.getElementById('suc_id').value=''; document.getElementById('suc_nom').value=''; }
...
function filtrarHist(){
 ...
 let fil=v.filter(x=>{
   let ok=true;
   if(fEmp) ok=ok && (x.empleado_id.toLowerCase().includes(fEmp) || x.sucursal_nombre.toLowerCase().includes(fEmp));
   if(fFecha) ok=ok && x.fecha===fFecha;
   return ok;
 });
 renderHist(fil);
}
async function cargarReporte(){
 const emps=await api('/empleados?empresa_id='+EMPRESA_ID);
 document.getElementById('reporte_box').innerHTML=emps.map(e=>{
   let sueldo=e.sueldo_monto||0;
   let descuento=(e.faltas* (sueldo/30)) + (e.retardos*10);
   let pagar=sueldo - descuento;
   if(e.sueldo_tipo!=='mes') pagar=sueldo;
   return `<div style="padding:8px;border-bottom:1px solid #1e293b;display:flex;justify-content:space-between"><div><b>${e.nombre}</b> (${e.id})<br><small>${e.sueldo_tipo} $${sueldo} - F:${e.faltas} R:${e.retardos} - ⭐${e.promedio_eval}</small></div><div style="text-align:right"><b style="color:#10b981">$${pagar.toFixed(0)}</b><br><small style="color:#f87171">Desc: $${descuento.toFixed(0)}</small></div></div>`;
 }).join('');
}
...
async function cargarDashboard(){
 ...
 // GRAFICAS
 try{
   let activos=emps.filter(e=>e.estatus==='activo').length;
   let baja=emps.filter(e=>e.estatus==='baja').length;
   let vac=emps.filter(e=>e.estatus==='vacaciones').length;
   let inc=emps.filter(e=>e.estatus==='incapacidad').length;
   let ctx1=document.getElementById('chartEstatus');
   if(ctx1){
     if(chart1) chart1.destroy();
     chart1=new Chart(ctx1,{type:'doughnut',data:{labels:['Activo','Baja','Vac','Incap'],datasets:[{data:[activos,baja,vac,inc],backgroundColor:['#10b981','#ef4444','#f59e0b','#6366f1']}]},options:{plugins:{legend:{labels:{color:'#e2e8f0'}}}}});
   }
   let topRet=emps.sort((a,b)=>b.retardos-a.retardos).slice(0,5);
   let ctx2=document.getElementById('chartRet');
   if(ctx2){
     if(chart2) chart2.destroy();
     chart2=new Chart(ctx2,{type:'bar',data:{labels:topRet.map(e=>e.nombre.split(' ')[0]),datasets:[{label:'Retardos',data:topRet.map(e=>e.retardos),backgroundColor:'#f59e0b'}]},options:{plugins:{legend:{display:false}},scales:{x:{ticks:{color:'#94a3b8'}},y:{ticks:{color:'#94a3b8'}}}}});
   }
 }catch(e){console.log('chart err',e)}
}
async function cambiarPassAdmin(){
 let nueva=document.getElementById('new_admin_pass').value;
 if(!nueva) return alert('Pon pass');
 let r=await api('/empresas/'+EMPRESA_ID+'/cambiar-pass','POST',{nueva:nueva});
 document.getElementById('msg_admin_pass').innerText=r.mensaje || r.error;
}
...
def checkin(v: VisitaCreate):
    ...
    if emp:
        try:
            ...
            pass
    visita=VisitaDB(empleado_id=v.empleado_id, sucursal_id=v.sucursal_id, sucursal_nombre=v.sucursal_nombre, es_retardo=es_retardo, notas=v.notas + (" - RETARDO" if es_retardo else " - A TIEMPO"), empresa_id=v.empresa_id)
...
def reporte_csv(empresa_id: str="DEFAULT"):
    ...
    db.close()
    csv="ID,Nombre,Puesto,Sucursal,SueldoTipo,Monto,Estatus,Faltas,Retardos,VacTomadas,VacTotales,Eval,Empresa,APagar,Descuento\n"
    for e in emps:
        sueldo=e.sueldo_monto or 0
        desc=(e.faltas*(sueldo/30) if sueldo else 0)+(e.retardos*10)
        pagar=sueldo-desc if e.sueldo_tipo=='mes' else sueldo
        csv+=f"{e.id},{e.nombre},{e.puesto},{e.sucursal},{e.sueldo_tipo},{e.sueldo_monto},{e.estatus},{e.faltas},{e.retardos},{e.vacaciones_tomadas},{e.vacaciones_totales},{e.promedio_eval},{e.empresa_id},{pagar:.0f},{desc:.0f}\n"
...
@app.delete("/sucursales/{suc_id}")
def del_suc(suc_id: str, empresa_id: str="DEFAULT"):
    db=SessionLocal()
    ex=db.query(SucursalDB).filter(SucursalDB.id==suc_id).first()
    if ex: db.delete(ex); db.commit()
    db.close(); return {"ok":True}

@app.post("/empresas/{emp_id}/cambiar-pass")
def cambiar_pass_empresa(emp_id: str, data: dict):
    db=SessionLocal()
    emp=db.query(EmpresaDB).filter(EmpresaDB.id==emp_id).first()
    if not emp: db.close(); return {"error":"Empresa no existe"}
    nueva=data.get("nueva","")
    if not nueva: db.close(); return {"error":"Vacía"}
    emp.admin_pass=nueva; db.commit(); db.close()
    return {"ok":True,"mensaje":"Pass admin cambiada ✅"}
