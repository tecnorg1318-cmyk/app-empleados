...
def registro_empresa(data: dict):
    ...
    # Fix: auto-verificar para no bloquear (demo)
...
<script>
// FIX ULTRA SIMPLE - Reemplaza login roto con version que SI funciona
async function login(){
  const u=document.getElementById('u').value.trim();
  const p=document.getElementById('p').value.trim();
  const msgEl=document.getElementById('msg');
  if(!u||!p){ msgEl.innerText='⚠️ Pon usuario y contraseña'; return; }
  msgEl.innerText='⏳ Ingresando...';
  try{
    ...
    localStorage.setItem('user_id',d.usuario||u);
    localStorage.setItem('rol',d.subrol||d.rol||'empleado');
    localStorage.setItem('nombre',d.nombre||u);
    USER_ID=d.usuario||u;
    ROL=d.subrol||d.rol||'empleado';
    if(d.rol==='admin' || ROL!=='empleado'){
      document.getElementById('admin-area').style.display='block';
      document.getElementById('empleado-area').style.display='none';
      ...
      document.getElementById('admin-area').style.display='none';
      document.getElementById('empleado-area').style.display='block';
      if(typeof cargarEmpleadoPro==='function') cargarEmpleadoPro();
      else if(typeof cargarEmpleado==='function') cargarEmpleado();
    }
    msgEl.innerText='';
  }catch(e){
    console.error('Login error', e);
    msgEl.innerText='❌ '+(e.detail||e.message||'Error, revisa usuario/contraseña');
    msgEl.innerHTML+='<br><small style="font-size:10px">Intenta: admin / admin123 o EMP0001 / 0001</small>';
  }
}
function mostrarRecuperar(){ 
  const id=prompt('Pon tu ID de empleado (ej: EMP0001 o admin):');
  if(!id) return;
  fetch('/api/recuperar-password',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({empleado_id:id})})
  .then(r=>r.json().then(d=>({ok:r.ok,data:d})))
  .then(res=>{ 
    if(res.ok) alert('✅ Nueva contraseña: '+res.data.nueva_password+'\n'+res.data.mensaje);
    else alert('❌ '+(res.data.detail||'Error'));
  })
  .catch(err=>alert('❌ Error: '+err));
}
function mostrarRegistro(){
  document.getElementById('login').style.display='none';
  document.getElementById('registro-empresa-modal').style.display='flex';
}
function mostrarLogin(){
  document.getElementById('registro-empresa-modal').style.display='none';
  document.getElementById('login').style.display='flex';
}
// Forzar que botones funcionen con click directo
document.addEventListener('DOMContentLoaded', ()=>{
  console.log('✅ Fix login cargado - botones deben funcionar');
  // Test
  window.testLogin = ()=>{ console.log('Test login function exists', typeof login); };
});
</script>
