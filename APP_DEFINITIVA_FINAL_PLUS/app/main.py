...
def enviar_push_fcm(token, titulo, cuerpo):
    """Envia push al empleado: Se te olvido marcar, etc"""
    try:
        import requests
        # Usa Firebase Admin SDK en produccion
        print(f"[PUSH FCM a {token}]: {titulo} - {cuerpo}")
        return True
    except Exception as e:
        print(e)
        return False

@app.post("/api/push/recordatorio")
def push_recordatorio(empleado_id: int, tipo: str):
    # El robot de faltas llama esto ademas de WhatsApp
    enviar_push_fcm(f"token_empleado_{empleado_id}", f"Recordatorio: {tipo}", f"No olvides marcar {tipo}")
    return {"ok":True}
