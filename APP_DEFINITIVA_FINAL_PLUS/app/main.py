...
<style>
#login{ display:flex !important; position:fixed !important; inset:0 !important; z-index:9999 !important; }
</style>
</head><body>

<script>
// EMERGENCIA - Si la URL tiene ?clear, borra todo
if(window.location.search.includes('clear') || window.location.hash.includes('clear')){
  localStorage.clear();
  sessionStorage.clear();
  console.log('🧹 Sesión borrada por ?clear');
  history.replaceState(null, '', window.location.pathname);
}
// AUTO-REPARACION - Si después de 1.5s sigue en blanco, fuerza login
setTimeout(()=>{
  try{
    ...
    if(!loginVisible && !appVisible){
      console.log('⚠️ Pantalla en blanco detectada - forzando login');
